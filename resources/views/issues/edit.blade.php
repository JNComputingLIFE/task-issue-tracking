...
<div class="max-w-xl mx-auto bg-white p-6 rounded-xl shadow-sm border border-gray-100">
    <h1 class="text-xl font-bold mb-6 text-gray-800">Modify Issue #{{ $issue->id }}</h1>

    <form action="{{ route('issues.update', $issue) }}" method="POST" class="space-y-4">
        @csrf
        @method('PUT')

        <div>
            <label class="block text-xs font-bold text-gray-600 uppercase mb-1">Project Alignment</label>
            <select name="project_id" class="w-full border border-gray-300 rounded-lg p-2.5 text-sm bg-white">
        ...
        <div>
            ...
            <input type="text" name="title" value="{{ old('title', $issue->title) }}" class="w-full border border-gray-300 rounded-lg p-2.5 text-sm focus:border-indigo-500 @error('title') border-red-500 @enderror">
            @error('title') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
        </div>

        <div>
            <label class="block text-xs font-bold text-gray-600 uppercase mb-1">Detailed Description</label>
            <textarea name="description" rows="4" class="w-full border border-gray-300 rounded-lg p-2.5 text-sm focus:border-indigo-500">{{ old('description', $issue->description) }}</textarea>
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <div>
                ...
                <input type="date" name="due_date" value="{{ old('due_date', $issue->due_date ? \Carbon\Carbon::parse($issue->due_date)->format('Y-m-d') : '') }}" class="w-full border border-gray-300 rounded-lg p-2.5 text-sm">
            </div>
        </div>
        ...
        <div class="flex justify-end space-x-3 pt-4">
            ...
            <button type="submit" class="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-semibold rounded-lg transition">Update Details</button>
        </div>
    </form>
</div>
@endsection